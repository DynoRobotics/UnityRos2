...
namespace RosSharp.Urdf.Editor
{
    public static class UrdfMaterialFactory
    {
        ...
        public static Dictionary<string, UrdfLink.Visual.Material> Materials =
            new Dictionary<string, UrdfLink.Visual.Material>();
        
        #region Import
        private static Material CreateMaterial(this UrdfLink.Visual.Material urdfMaterial)
        {
        ...
        private static string GenerateMaterialName(UrdfLink.Visual.Material urdfMaterial)
        {
            var materialName = "";
            if (urdfMaterial.color != null)
            {
        ...
        private static Color CreateColor(UrdfLink.Visual.Material.Color urdfColor)
        {
            return new Color(
        ...
        public static void SetUrdfMaterial(GameObject gameObject, UrdfLink.Visual.Material urdfMaterial)
        {
        ...
        public static UrdfLink.Visual.Material ExportMaterialData(Material material)
        {
            ...
            if (!Materials.ContainsKey(material.name))
            {
                ...
                {
                    UrdfLink.Visual.Material.Texture texture = ExportTextureData(material.mainTexture);
                    Materials[material.name] = new UrdfLink.Visual.Material(material.name, null, texture);
                }
                else if (!material.color.Equals(Color.clear))
                {
                    UrdfLink.Visual.Material.Color color = new UrdfLink.Visual.Material.Color(ExportRgbaData(material));
                    Materials[material.name] = new UrdfLink.Visual.Material(material.name, color);
                }
                else
                    return null;
            }

            return new UrdfLink.Visual.Material(material.name);
        }
        ...
        private static UrdfLink.Visual.Material.Texture ExportTextureData(Texture texture)
        {
            ...
            return new UrdfLink.Visual.Material.Texture(packagePath);
        }
